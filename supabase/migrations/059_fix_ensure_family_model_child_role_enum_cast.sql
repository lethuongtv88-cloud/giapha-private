-- Fix: CASE WHEN ... THEN 'husband' ... END trong ensure_family_model_child
-- được Postgres suy luận kiểu là `text` (không tự động theo kiểu cột đích
-- family_parents.role là enum parent_role_enum), gây lỗi:
--   column "role" is of type parent_role_enum but expression is of type text
-- Chỉ cần ép kiểu tường minh ::parent_role_enum cho cả 2 CASE (parent_a và
-- parent_b), giữ nguyên toàn bộ logic còn lại của hàm.
