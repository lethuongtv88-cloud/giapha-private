-- 066_fix_get_single_active_spouse_uuid_min.sql
--
-- FIX lỗi khi apply migration 064:
--   ERROR: 42883: function min(uuid) does not exist
--   LINE: SELECT CASE WHEN COUNT(*) = 1 THEN MIN(spouse_id) ELSE NULL END
--
-- Nguyên nhân: PostgreSQL không có aggregate MIN()/MAX() built-in cho kiểu
-- uuid. Vì logic chỉ cần lấy giá trị DUY NHẤT khi COUNT(*) = 1 (không
-- quan tâm thứ tự), thay MIN(spouse_id) bằng (array_agg(spouse_id))[1].
--
-- Vì 064 gói toàn bộ trong 1 transaction (BEGIN...COMMIT) nên khi lỗi xảy
-- ra, CẢ HAI function (get_single_active_spouse VÀ
-- repair_single_parent_family_children) đều KHÔNG được tạo/cập nhật.
-- Migration này tạo lại đầy đủ cả hai, y hệt nội dung 064 nhưng đã sửa
-- lỗi MIN(uuid).
...
AS $$
  SELECT CASE WHEN COUNT(*) = 1 THEN (array_agg(spouse_id))[1] ELSE NULL END
  FROM (
    SELECT DISTINCT
